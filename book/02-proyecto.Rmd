...
A primera vista podemos apreciar que el porcentaje pagado no siempre suma 100%; por ahora ignoraremos esto ya que en la sección \<ref\> se cubre la limpieza de datos. Poniendo atención al comportamiento de pago representado en la imagen tenemos que:
...
En el \<Anexo\> se incluye el detalle teórico de la construcción de pesos con *decaimiento geométrico* para lo que se utilizan, precisamente, distribuciones geométricas. A partir de series de decaimiento geométrico generados con ese método y series de saldo de los clientes generamos las variables:
...
En la figura \<ref\> tenemos las series de diferencias y de desviación estándar del saldo de un crédito. 
...
La siguiente lista muestra algunas de las variables que se generaron:
...
* `saldo_prom_decay` - promedios móviles del saldo histórico, ponderados con decaimiento geométrico y half-life de tres meses.
* `saldo_prom_decay3` - promedios móviles del saldo histórico, ponderados con decaimiento geométrico y half-life de tres cortes.
* `saldo_sd_decay3` - desviación estándar del saldo histórico de un cliente, ponderado con decaimiento geométrico y half-life de tres cortes.
...
En el Anexo \<ref\> se incluye la lista completa de variables que se utilizaron en la selección de modelos. En github \<ref\> se encuentra el código con el que se construyeron esas variables. 

### Etiquetas 

Recordemos que en el capítulo \<ref\> se dijo que el indicador de impuntualidad debe predecir si, en los siguientes tres cortes, el momento de pago de un cliente cae en la cola amarilla y roja del tríangulo de pagos. 
...
Como se mencionó en la Sección \<ref\> la proporción de positivos y negativos en las etiquetas históricas es de 90.3% y 9.7%. Las proporciones en los tres conjuntos de cross-validation se mantuvieron muy cerca de estos valores.
...
Hagamos un ejercicio visual simple que nos permita ver cómo el decaimiento ayuda a capturar el patrones de pago de los clientes. La figura \<ref\> contiene dos gráficas de componentes principales. La diferencia en las gráficas es que los puntos se generaron con distintas matrices de rotación:
...
Finalmente, veamos la curva roc de tres algoritmos de clasificación: regresión logística con penalización L2, bosque aleatorio y gradient boosting classifier. Los tres algoritmos fueron entrenados sobre el conjunto de datos `long` con variables de datos puntuales y con decaimiento.
...
En la sección \<ref\> sobre métricas de validación, establecimos que para aprobar un modelo, este debía alcanzar un recall de 50% y una especificidad de 93%. A continuación mostramos las métricas de desempeño del modelo puesto en producción.

Ya mencionamos que el modelo elegido fue un Gradient Boosting Classifier. Este tipo de algoritmo realmente no requiere que se elija un subconjunto de variables \<bib\>, por lo que este se entrenó con todo el conjunto de variables que presentamos en el Anexo \<ref\>. La selección de hiperparámetros consistió basicamente en elegir la profundidad de los árboles con un grid. El número de árboles quedó determinado por el early stopping que está incorporado al algoritmo utilizado.

A continuación mostramos diferentes medidas desempeño del modelo y corte (threshold) elegidos. Por corte nos referimos al valor a partir del cual se le asigna a la predicción el valor de uno o cero.  
...
* precision 65.3% - Ya que el objetivo del modelo de predicción es accionar estrategias de cobranza correctivas es importante tomar en cuenta cuántos recursos se destinan a este esfuerzo. En ese sentido es importante considerar la precisión del modelo elegido para evitar gastos innecesarios.
* accuracy 92.3% - Este nivel de accuracy excede lo requerido satisfactoriamente.

Después de que el Banco aprobó el desempeño del modelo entrenado, se refactorizó el código y se puso en producción un pipeline de datos. De esta manera se empezaron a producir indicadores de impuntualidad para los clientes activos. 
...
|Variable                            |etiqueta positiva|etiqueta negativa|
|------------------------------------|-----------------|-----------------|
|`dias_despues_fechaPago_prom_decay3`|1.23             |-0.7             |
|`dias_despues_fechaPago`            |1                |0                |
|`porc_mora_prom_decay3`             |0.35             |0                |
|`porc_puntual_prom_decay3`          |0.68             |1.04             |
|`porc_puntual`                      |0.56             |1.04             |
|`saldoActual`                       |54,848           |54,290           |
|`antiguedad`                        |397              |548              |

Con esto podemos decir que `dias_despues_fechaPago_prom_decay3` el promedio con decaimiento de los momentos de pago es la variable que más explica el comportamiento futuro de pago de los clientes. Como era de esperarse, si un cliente se ha atrasado en sus pagos en el pasado, es probable que vuelva a ser impuntual en el futuro.
